    ...
    buildDecorations(view: EditorView): DecorationSet {
        ...
        tree.iterate({
            ...
            enter(node) {
                ...
                if (!link) return
                builder.add(node.from, node.from + LINK_START.length, Decoration.mark({
                    class: 'cm-formatting-link cm-formatting-link_formatting-link-start',
                    tagName: 'span',
                }))
                builder.add(node.from + LINK_START.length, node.to - LINK_END.length, Decoration.mark({
                    class: 'cm-hmd-internal-link',
                    tagName: 'span',
                }))
                builder.add(node.to - LINK_END.length, node.to, Decoration.mark({
...
export const frontmatterLinkPlugin = ViewPlugin.fromClass(FrontmatterLinkPlugin, pluginSpec)
