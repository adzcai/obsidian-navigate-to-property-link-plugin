...
import { App, type FrontmatterLinkCache, FuzzySuggestModal, Notice, Plugin, TFile, MarkdownRenderChild, parseYaml, resolveSubpath, type CachedMetadata } from 'obsidian';
import { mount, unmount } from 'svelte';
import { type Props, type Column, columnKey, type Row, LINK_START, LINK_END, LINK_KEY, linkMatch, Periodicity } from 'src/utils';
	...
	async onload() {
		...
		this.addCommand({
			...
			editorCheckCallback: (checking, editor, ctx) => {
				...
				const { from, to, value: decoration } = plugin.decorations.iter(cursor);
				if (!decoration || cursor < from) return false;
				const dest = editor.getRange(
					editor.offsetToPos(from),
					editor.offsetToPos(to),
				);
				if (dest === LINK_START || dest === LINK_END) return false;
